...
VRAMStats Profiler::GetTotalVRAMUsage(VkPhysicalDevice device, VmaAllocator allocator) {
	...
	for (uint32_t i = 0; i < memProps.memoryHeapCount; ++i) {
		...
		if (isDeviceLocal) {
			stats.used += budgets[i].usage;
			stats.budget += budgets[i].budget;

			fmt::println("[Heap {}] Device-local | Usage: {} MB / Budget: {} MB",
				i,
				budgets[i].usage / (1024ull * 1024ull),
				budgets[i].budget / (1024ull * 1024ull));
		}
		else {
			fmt::println("[Heap {}] Non-local | Usage: {} MB / Budget: {} MB",
				i,
				budgets[i].usage / (1024ull * 1024ull),
				budgets[i].budget / (1024ull * 1024ull));
		}

		fmt::println("[Heap {}] Flags = 0x{:x}, Size = {} MB",
			i,
			memProps.memoryHeaps[i].flags,
			memProps.memoryHeaps[i].size / (1024ull * 1024ull));
	}

	fmt::println("Total VRAM (device-local): {} MB / {} MB",
		stats.used / (1024ull * 1024ull),
		stats.budget / (1024ull * 1024ull));
