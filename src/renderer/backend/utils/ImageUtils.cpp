	...
	bool skipQueueUsage)
{
	...
	AllocatedBuffer uploadBuffer = BufferUtils::CreateBuffer(
		...
		allocator);
	memcpy(uploadBuffer.m_allocInfo.pMappedData, data, dataSize);
	...
	CommandBuffer::RecordDeferredCmd([&](VkCommandBuffer cmd) {
		...
		if (renderImage.bIsMipmapped)
		{
			generateMipmaps(cmd, renderImage);
		}
		else {
			transitionImage(
				cmd,
	...
	auto buffer = uploadBuffer.m_buffer;
	auto bufAlloc = uploadBuffer.m_allocation;
	auto alloc = allocator;
	bufferQueue.PushFunction([buffer, bufAlloc, alloc] {
	...
	bool skipDQ)
{
	...
	if (renderImage.bIsMipmapped && renderImage.mipLevelCount == 0) {
...

	{
		...
		// Default path
			...
			viewInfo.viewType = renderImage.bIsCubemap ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_2D;
		}
		...
		if ((usage & VK_IMAGE_USAGE_STORAGE_BIT) && (samples & VK_SAMPLE_COUNT_1_BIT)) {
			...
			if (imgInfo.mipLevels > 1 && renderImage.bPerMipStorageViews) {
			...
			else if (imgInfo.arrayLayers > 1 || renderImage.bIsCubemap) {
	...
	if (!skipDQ) {
		...
		dq.PushFunction([device, image, alloc, imgView, v_storageViews, v_layerViews, imgAlloc] {
...
void ImageUtils::destroyImage(VkDevice device, AllocatedImage& img, const VmaAllocator allocator) {
	static std::mutex imageMutex;
	std::scoped_lock lock(imageMutex);
...
void ImageUtils::generateCubemapMiplevels(VkCommandBuffer cmd, AllocatedImage& image) {
	assert(image.bIsCubemap && "generateCubemapMiplevels requires a cubemap image");
...
VkImageView ImageUtils::CreateImageView(VkDevice device, VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
void ImageUtils::generateMipmaps(VkCommandBuffer cmd, const AllocatedImage& image) {
	...
	// cube mip mapping
	uint32_t layerCount = image.bIsCubemap ? 6 : 1;
	...
	VkBorderColor borderColor)
{
	...
	if (dQueue != nullptr) {
		dQueue->PushFunction([sampler, device] {
