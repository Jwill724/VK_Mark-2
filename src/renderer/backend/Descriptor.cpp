...
void DescriptorManager::InitDescriptors(VkDevice device)
{
	...
	AddBinding(RD::GLOBAL_BINDING_ENV_INDEX, UNIFORM, ALL_STAGES);
	...
	size_t pushSetID = static_cast<size_t>(RD::DescriptorSlot::Push);
	m_descriptorDefs[pushSetID].descriptorLayout = CreatePushSetLayout(device);
}
...
VkDescriptorSet DescriptorManager::AllocateFrameDescriptorSet(VkDevice device)
{
	...
	ASSERT(frameLayout != VK_NULL_HANDLE);
